...
class Campaign extends Model
{
   ...
   public function shouldWeUseIt($receivedMessage, Contact $contact) //Brij Mohan Negi Update
    ...
    public function makeMessages($request,Contact $contact=null){
        ...
        if ($request && $request->has('contact_id') && is_array($request->contact_id)) {
            $contacts = Contact::whereIn('id', $request->contact_id)->get();
        } else if ($request && $request->has('group_id') && is_array($request->group_id)) {
            $contacts = Contact::whereHas('groups', function($q) use ($request) {
                $q->whereIn('groups_contacts.group_id', $request->group_id);
