                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('knowledge.frontend.category', [$company->subdomain, $article->category->slug]) }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-gray-500 dark:text-gray-400 truncate">{{ $article->title }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/4 lg:order-2">
                ...
                <!-- Article Info -->
                <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6 transition-colors duration-200">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Article Info</h3>
                    <div class="space-y-3">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6 transition-colors duration-200">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('knowledge.frontend.article', [$company->subdomain, $relatedArticle->slug]) }}" 
                           class="block group">
                            <h4 class="text-sm font-medium text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2">
                                {{ $relatedArticle->title }}
                            </h4>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $relatedArticle->formatted_read_time }}</p>
                        </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 transition-colors duration-200">
                    ...
                    <nav class="space-y-1">
                        ...
                        <a href="{{ route('knowledge.frontend.category', [$company->subdomain, $cat->slug]) }}" 
                           class="group flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ $cat->id == $article->category_id ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-gray-100' }}"
                            <span>{{ $cat->name }}</span>
                            <span class="text-xs {{ $cat->id == $article->category_id ? 'text-indigo-500 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500' }}">
            ...
            <!-- Main Content -->
            <div class="lg:w-3/4 lg:order-1">
                <article class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden transition-colors duration-200">
                    <!-- Article Header -->
                    <div class="px-8 py-8 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1">
                                @if($article->category)
                                    <a href="{{ route('knowledge.frontend.category', [$company->subdomain, $article->category->slug]) }}" 
                            ...
                            <div class="ml-4">
                                <button onclick="copyToClipboard()" class="p-2 text-gray-400 hover:text-gray-600 transition-colors" title="Copy link">
                        ...
                        @if($article->excerpt)
                            <p class="text-lg text-gray-600 dark:text-gray-300 leading-relaxed">{{ $article->excerpt }}</p>
                        @endif
                    </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex space-x-2">
                                    <button onclick="helpful(true)" class="p-1 text-gray-400 hover:text-green-600 dark:text-gray-500 dark:hover:text-green-400 transition-colors" title="Yes">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"></path>
                                        </svg>
                                    </button>
                                    <button onclick="helpful(false)" class="p-1 text-gray-400 hover:text-red-600 dark:text-gray-500 dark:hover:text-red-400 transition-colors" title="No">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-8 flex justify-between">
                    ...
                    <a href="{{ route('static-page', $company->subdomain) }}" 
                       class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors">
                        All Categories
...
<script>
    ...
    function helpful(isHelpful) {
        // Here you could make an AJAX call to track helpful feedback
        const message = isHelpful ? 'Thank you for your feedback!' : 'Thanks, we\'ll work to improve this article.';
        const color = isHelpful ? 'bg-green-500' : 'bg-blue-500';
        
        const toast = document.createElement('div');
        toast.className = `fixed top-4 right-4 ${color} text-white px-4 py-2 rounded-lg shadow-lg z-50`;
